...
public class Function {
	...
	private double getSampleValue(Object obj, int[] sample, int depth) {
		if(depth == (this.variables.size()-1)) {
			...
			return getSampleValue(((Object[]) obj)[sample[this.variables.get(depth).index]], sample, depth+1);
		}
	}
	
	public void updateSampleValue(Object obj, int[] sample, double value, int depth) {
		if(depth == (this.variables.size()-1)) {
			((Object[]) obj)[sample[this.variables.get(depth).index]] = (Double)((Object[]) obj)[sample[this.variables.get(depth).index]] + value;
		} else {
			updateSampleValue(((Object[]) obj)[sample[this.variables.get(depth).index]], sample, value, depth+1);
		}
	}
